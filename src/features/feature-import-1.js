// Import handling for DictBot.
const { EventEmitter } = require('node:events');

class ImportHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'import' + ' payload');
 return { 'import': String(payload).slice(0, 200) };
 }
...
module.exports = { ImportHandler, run: (payload, opts) => new ImportHandler().run(payload, opts) };