// Validation handling for DictBot.
const { EventEmitter } = require('node:events');

class ValidationHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'validation' + ' payload');
 return { 'validation': String(payload).slice(0, 200) };
 }
...
module.exports = { ValidationHandler, run: (payload, opts) => new ValidationHandler().run(payload, opts) };